 ...
 *    Mmemory model Changed to eliminate global workspaces; to allow parallelization
 *    Supressed the function InitializeRKF78Sys which, now, is not necessary
 * Version 2.1, June 20, 2020.
 *    Data type of vector field dimension changed to unsigned to allow larger vector fields
 * Version 2.0, May 27, 2020.
 *    Changes for consistency between RKF78 and RKF78Sys
 *    Corrected error in error control in RKF78Sys
 * Version 1.2.3, May 26, 2020.
 *    Acceleration by eliminating loops and some stressed pointers.
 *    As a consequence, BLAS level 1 functions eliminated.
 * Version 1.2.2, May 25, 2020.
 *    Deep changes in the internal workspace memory model in RKF78Sys
 *    Introduced two real BLAS functions for RKF78Sys
 *    Other minor formatting improvements
 * Version 1.2.1, April 25, 2020. Introduced explicit report of internal
 *                              errors and minor formatting improvements
 * Version 1.2, April 11, 2020. Implements the control of floating
 *                             overflows in the predictions.
 *
 * Version 1.1 Jan 15, 2020. It corrects the error:
 ...
 *   VField_dim: Only in RKF78Sys; It is de dimension of x
 ...
 *            This function MUST be defined with the following header:
        ...
        For RKF78:     void (*ODE)(double, double, double *, void *)
        For RKF78Sys:  void (*ODE_Sys)(double, double *, unsigned, double *, void *)

    or, for example (names are not compulsory),

                void ODE_name(double t, double x, double *f, void *Params)
                void ODE_sys_name(double t, double *x, unsigned VField_dim, double *f, void *Params)
 ...
 *             VField_dim is the dimension of the vector field; x; and f
 *             f -- output: is the evaluation of the ODE at time t on the point x=x(t)
 *             Params -- input: must be set to
...
/* Computation of h * k_0. Special case since x is not displaced */
        ODE(*t, *x, ksub, ParmsStruct); ksub[0] = ksub[0] * *h; // k_0 = f(t, x); k_0 -> h * k_0

/* Computation of h * k_1 */
        x7pred = *x + beta[0] * ksub[0];
        ODE(*t + alpha[1]* *h, x7pred, ksub+1, ParmsStruct); ksub[1] = ksub[1] * *h; //  k_1 = f(t + alpha[1]*h, x + beta[0]*(h * k_0)); k_1 -> h * k_1

/* Computation of h * k_2 */
        x7pred = *x + beta[1] * ksub[0] + beta[2] * ksub[1];
        ODE(*t + alpha[2]* *h, x7pred, ksub+2, ParmsStruct); ksub[2] = ksub[2] * *h; //  k_2 =  f(t + alpha[2]*h, x + beta[1] (h * k_0) + beta[2] (h * k_1)); k_2 -> h * k_2

/* Computation of h * k_3. Special case: b_{4,2} = beta[4] = 0. To avoid multiplying by 0 */
        x7pred = *x + beta[3] * ksub[0] + beta[5] * ksub[2];
        ODE(*t + alpha[3]* *h, x7pred, ksub+3, ParmsStruct); ksub[3] = ksub[3] * *h;

/* Computation of h * k_4. Special case: b_{5,2} = beta[7] = 0. To avoid multiplying by 0 */
        x7pred = *x + beta[6] * ksub[0] + beta[8] * ksub[2] + beta[9] * ksub[3];
        ODE(*t + alpha[4]* *h, x7pred, ksub+4, ParmsStruct); ksub[4] = ksub[4] * *h;

/* Computation of h * k_i; i >= 5. Normal cases: b_{i+1,2} = b_{i+1,3} = 0 */
        x7pred = *x + beta[10] * ksub[0] + beta[13] * ksub[3] + beta[14] * ksub[4];
        ODE(*t + alpha[5]* *h, x7pred, ksub+5, ParmsStruct); ksub[5] = ksub[5] * *h;

        x7pred = *x + beta[15] * ksub[0] + beta[18] * ksub[3] + beta[19] * ksub[4] + beta[20] * ksub[5];
        ODE(*t + alpha[6]* *h, x7pred, ksub+6, ParmsStruct); ksub[6] = ksub[6] * *h;

        x7pred = *x + beta[21] * ksub[0] + beta[25] * ksub[4] + beta[26] * ksub[5] + beta[27] * ksub[6];
        ODE(*t + alpha[7]* *h, x7pred, ksub+7, ParmsStruct); ksub[7] = ksub[7] * *h;

        x7pred = *x + beta[28] * ksub[0] + beta[31] * ksub[3] + beta[32] * ksub[4] + beta[33] * ksub[5] + beta[34] * ksub[6] + beta[35] * ksub[7];
        ODE(*t + alpha[8]* *h, x7pred, ksub+8, ParmsStruct); ksub[8] = ksub[8] * *h;

        x7pred = *x + beta[36] * ksub[0] + beta[39] * ksub[3] + beta[40] * ksub[4] + beta[41] * ksub[5] + beta[42] * ksub[6] + beta[43] * ksub[7] + beta[44] * ksub[8];
        ODE(*t + alpha[9]* *h, x7pred, ksub+9, ParmsStruct); ksub[9] = ksub[9] * *h;

        x7pred = *x + beta[45] * ksub[0] + beta[48] * ksub[3] + beta[49] * ksub[4] + beta[50] * ksub[5] + beta[51] * ksub[6] + beta[52] * ksub[7] + beta[53] * ksub[8] + beta[54] * ksub[9];
        ODE(*t + alpha[10]* *h, x7pred, ksub+10, ParmsStruct); ksub[10] = ksub[10] * *h;

        x7pred = *x + beta[55] * ksub[0] + beta[60] * ksub[5] + beta[61] * ksub[6] + beta[62] * ksub[7] + beta[63] * ksub[8] + beta[64] * ksub[9];
        ODE(*t + alpha[11]* *h, x7pred, ksub+11, ParmsStruct); ksub[11] = ksub[11] * *h;

        x7pred = *x + beta[66] * ksub[0] + beta[69] * ksub[3] + beta[70] * ksub[4] + beta[71] * ksub[5] + beta[72] * ksub[6] + beta[73] * ksub[7] + beta[74] * ksub[8] + beta[75] * ksub[9] + ksub[11];
        ODE(*t + alpha[12]* *h, x7pred, ksub+12, ParmsStruct); ksub[12] = ksub[12] * *h;

/*    Computing the rk7 and rk8 predictions. We use that
 *                c7[1] = c7[2] = c7[3] = c7[4] = 0, and
 *        c8[0] = c8[1] = c8[2] = c8[3] = c8[4] = c8[10] = 0 */
        x7pred = *x + c7[0] * ksub[0] + c7[5] * ksub[5] + c7[6] * ksub[6] + c7[7] * ksub[7] + c7[8] * ksub[8] + c7[9] * ksub[9] + c7[10] * ksub[10];
        x8pred = *x + c8[5] * ksub[5] + c8[6] * ksub[6] + c8[7] * ksub[7] + c8[8] * ksub[8] + c8[9] * ksub[9] + c8[11] * ksub[11] + c8[12] * ksub[12];

        if(isnan(x7pred) || isnan(x8pred)) return 66;

        *err = fabs(x8pred-x7pred); // Prediction error
        tolr = tol * (1.0 + fabs(x8pred)/100.0); /* It is an absolute tolerance for small values of the components and relative but "retarded two digts" for large values */
...
int RKF78Sys( double *t, double x[], unsigned VField_dim,
              double *h, double *err,
              double hmin, double hmax, double tol,
              void *ParmsStruct,
              void (*ODE_Sys)(double, double *, unsigned, double *, void *))
{   register unsigned i;
    double ksub_space[13][VField_dim], x7pred[VField_dim], x8pred[VField_dim];
    double *ksub[13] = { &ksub_space[0][0],  &ksub_space[1][0],  &ksub_space[2][0],
 ...
 * Using the vector x7pred as intermediate storage */
    while (1) {
        ODE_Sys(*t, x, VField_dim, ksub[0], ParmsStruct); // k_0 = f(t, x)

        for(i=0; i < VField_dim; i++){ ksub[0][i] =  ksub[0][i] * *h; x7pred[i] = x[i] + beta[0] * ksub[0][i]; } // k_0 -> h * k_0 &  x7pred = x + beta[0] (h * k_0)
        ODE_Sys(*t + alpha[1]* *h, x7pred, VField_dim, ksub[1], ParmsStruct); // k_1 = f(t + alpha[1]*h, x + beta[0]*(h * k_0));

        for(i=0; i < VField_dim; i++){ ksub[1][i] =  ksub[1][i] * *h; x7pred[i] = x[i] + beta[1] * ksub[0][i] + beta[2] * ksub[1][i]; } // k_1 -> h * k_1 &  x7pred = x + beta[1] (h * k_0) + beta[2]*(h * k_1)
        ODE_Sys(*t + alpha[2]* *h, x7pred, VField_dim, ksub[2], ParmsStruct); // k_2 = f(t + alpha[2]*h, x + beta[1] (h * k_0) + beta[2] (h * k_1));

/* Computation of h * k_3. Special case: b_{4,2} = beta[4] = 0. To avoid multiplying by 0 */
        for(i=0; i < VField_dim; i++){ ksub[2][i] =  ksub[2][i] * *h; x7pred[i] = x[i] + beta[3] * ksub[0][i] + beta[5] * ksub[2][i]; } // k_2 -> h * k_2 & x7pred = x + beta[3] (h * k_0) + beta[5]*(h * k_2)
        ODE_Sys(*t + alpha[3]* *h, x7pred, VField_dim, ksub[3], ParmsStruct); // k_3 = f(t + alpha[3]*h, x + beta[3] (h * k_0) + beta[5] (h * k_2));

/* Computation of h * k_4. Special case: b_{5,2} = beta[7] = 0. To avoid multiplying by 0 */
        for(i=0; i < VField_dim; i++){ ksub[3][i] =  ksub[3][i] * *h; x7pred[i] = x[i] + beta[6] * ksub[0][i] + beta[8] * ksub[2][i] + beta[9] * ksub[3][i]; } // k_3 -> h * k_3 & x7pred = x + beta[6] (h * k_0) + beta[8]*(h * k_2) + beta[9]*(h * k_3)
        ODE_Sys(*t + alpha[4]* *h, x7pred, VField_dim, ksub[4], ParmsStruct); // k_4 = f(t + alpha[4]*h, x + beta[6] (h * k_0) + beta[8] (h * k_2) + beta[9]*(h * k_3));

/* Computation of h * k_i; i >= 5. Normal cases: b_{i+1,2} = b_{i+1,3} = 0 */
        for(i=0; i < VField_dim; i++){ ksub[4][i] = ksub[4][i] * *h; x7pred[i] = x[i] + beta[10] * ksub[0][i] + beta[13] * ksub[3][i] + beta[14] * ksub[4][i]; }
        ODE_Sys(*t + alpha[5]* *h, x7pred, VField_dim, ksub[5], ParmsStruct);

        for(i=0; i < VField_dim; i++){ ksub[5][i] = ksub[5][i] * *h; x7pred[i] = x[i] + beta[15] * ksub[0][i] + beta[18] * ksub[3][i] + beta[19] * ksub[4][i] + beta[20] * ksub[5][i]; }
        ODE_Sys(*t + alpha[6]* *h, x7pred, VField_dim, ksub[6], ParmsStruct);

        for(i=0; i < VField_dim; i++){ ksub[6][i] = ksub[6][i] * *h; x7pred[i] = x[i] + beta[21] * ksub[0][i] + beta[25] * ksub[4][i] + beta[26] * ksub[5][i] + beta[27] * ksub[6][i]; }
        ODE_Sys(*t + alpha[7]* *h, x7pred, VField_dim, ksub[7], ParmsStruct);

        for(i=0; i < VField_dim; i++){ ksub[7][i] = ksub[7][i] * *h; x7pred[i] = x[i] + beta[28] * ksub[0][i] + beta[31] * ksub[3][i] + beta[32] * ksub[4][i] + beta[33] * ksub[5][i] + beta[34] * ksub[6][i] + beta[35] * ksub[7][i]; }
        ODE_Sys(*t + alpha[8]* *h, x7pred, VField_dim, ksub[8], ParmsStruct);

        for(i=0; i < VField_dim; i++){ ksub[8][i] = ksub[8][i] * *h; x7pred[i] = x[i] + beta[36] * ksub[0][i] + beta[39] * ksub[3][i] + beta[40] * ksub[4][i] + beta[41] * ksub[5][i] + beta[42] * ksub[6][i] +
                                                                                        beta[43] * ksub[7][i] + beta[44] * ksub[8][i]; }
        ODE_Sys(*t + alpha[9]* *h, x7pred, VField_dim, ksub[9], ParmsStruct);

        for(i=0; i < VField_dim; i++){ ksub[9][i] = ksub[9][i] * *h; x7pred[i] = x[i] + beta[45] * ksub[0][i] + beta[48] * ksub[3][i] + beta[49] * ksub[4][i] + beta[50] * ksub[5][i] + beta[51] * ksub[6][i] +
                                                                                        beta[52] * ksub[7][i] + beta[53] * ksub[8][i] + beta[54] * ksub[9][i]; }
        ODE_Sys(*t + alpha[10]* *h, x7pred, VField_dim, ksub[10], ParmsStruct);

        for(i=0; i < VField_dim; i++){ ksub[10][i] = ksub[10][i] * *h; x7pred[i] = x[i] + beta[55] * ksub[0][i] + beta[60] * ksub[5][i] + beta[61] * ksub[6][i] + beta[62] * ksub[7][i] + beta[63] * ksub[8][i] + beta[64] * ksub[9][i]; }
        ODE_Sys(*t + alpha[11]* *h, x7pred, VField_dim, ksub[11], ParmsStruct);

        for(i=0; i < VField_dim; i++){ ksub[11][i] = ksub[11][i] * *h; x7pred[i] = x[i] + beta[66] * ksub[0][i] + beta[69] * ksub[3][i] + beta[70] * ksub[4][i] + beta[71] * ksub[5][i] + beta[72] * ksub[6][i] +
                                                                                          beta[73] * ksub[7][i] + beta[74] * ksub[8][i] + beta[75] * ksub[9][i] + ksub[11][i]; }
        ODE_Sys(*t + alpha[12]* *h, x7pred, VField_dim, ksub[12], ParmsStruct);

/*    Computing the rk7 and rk8 predictions; and the norms.
 *    We use that c7[1] = c7[2] = c7[3] = c7[4] = 0, and
 *        c8[0] = c8[1] = c8[2] = c8[3] = c8[4] = c8[10] = 0   */
        *err = nor = 0.0;
        for(i=0; i < VField_dim; i++){ ksub[12][i] = ksub[12][i] * *h;
            x7pred[i] = x[i] + c7[0] * ksub[0][i] + c7[5] * ksub[5][i] + c7[6] * ksub[6][i] + c7[7] * ksub[7][i] + c7[8] * ksub[8][i] + c7[9] * ksub[9][i] + c7[10] * ksub[10][i];
            x8pred[i] = x[i] +                      c8[5] * ksub[5][i] + c8[6] * ksub[6][i] + c8[7] * ksub[7][i] + c8[8] * ksub[8][i] + c8[9] * ksub[9][i] +
                                                                                                                                        c8[11] * ksub[11][i] + c8[12] * ksub[12][i];
            nor += fabs(x8pred[i]); // L^1 norm of the prediction. Used to relativize tolerance
            *err += fabs(x8pred[i]-x7pred[i]);
        }

        if(isnan(nor) || isnan(*err)) return 66;

        *err /= VField_dim; // average of the component errors
        tolr = tol * (1.0 + nor/100.0); /* It is an absolute tolerance for small values of the components and relative but "retarded two digts" for large values */
    ...
    for(i=0; i < VField_dim; x[i] = x8pred[i], i++);
