...
/*
 * subroutines calculating the deterministic optimisation every some steps
 * using gsl functions
 *
 */
...
double __fitness(
	...
	double fitness = 0.0;
	if (run_runge_kutta(ic, params, *func, &fitness))
		return DBL_MAX;
...
double __central_difference(
    ...
    if (run_runge_kutta(params->ic, params->params, *(params->ff), &fitness))
        return DBL_MAX;
...
void __fitness_f_gradient(
	...
	if (run_runge_kutta(ic, params, *func, f)) *f = DBL_MAX;
